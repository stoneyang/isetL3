function [nIdeal, camera, srgbResult, srgbIdeal, raw] = ...
    cameraComputesrgb_RGBN(camera, sceneName, meanLuminance, sz, scenefov, ...
...
nIdeal = xyznIdeal(:, :, 4);
xyzIdeal = xyznIdeal(:, :, 1:3);

xyzIdeal  = xyzIdeal / max(xyzIdeal(:)) * scaleoutput;
[srgbIdeal, lrgbIdeal] = xyz2srgb(xyzIdeal);

%% Calculate camera result
[camera, lrgbResult] = cameraCompute_RGBN(camera,'oi',lrgbIdeal);   % OI is already calculated
