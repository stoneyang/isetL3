function vci = vcimageCompute_RGBN(vci,sensor)
...
if strcmpi(imageGet(vci,'name'),'default') || ... 
    strcmpi(imageGet(vci,'name'),'copy')
    vci = imageSet(vci,'name',sensorGet(sensor(1),'name'));
end
...
if length(sensor) > 1
    ...
    % individual sensors.
    vci = imageSet(vci,'datamax',sensorGet(sensor(1),'max'));
...
% continuous voltages 
vci = imageSet(vci,'input',sensorGet(sensor,'dvorvolts'));
...
switch pType
    case {'l3','l3local','l3global'}
        ...
        L3 = imageGet(vci,'L3');
        [L3xyzn,lumIdx,satIdx,clusterIdx] = L3render(L3,sensor,mode); 
        L3xyz = L3xyzn(:, :, 1:3);
        L3n = L3xyzn(:, :, 4);
        L3.L3n = L3n;
...
if nFilters == 1 && nSensors == 1
    ...
    vci = imageSet(vci,'result',repmat(img,[1,1,3]));
    return;
...
elseif nFilters >= 3 || nSensors > 1
    ...
    vci = imageSet(vci,'sensor space',img);
    ...
    % current matrix.
    tMethod = ieParamFormat(imageGet(vci,'transform method'));
    switch tMethod
        case 'current'
            % Use the stored transform matrix, don't recompute.
            T   = imageGet(vci,'prodT');
        ...
        case {'new','manual matrix entry'}
            ...
            Torig = imageGet(vci,'combined transform');
            Torig = Torig/max(Torig(:));
            T = ieReadMatrix(Torig,'%.3f   ','Color Transform');
            if isempty(T), return; end

            % Store and apply this transform.
            vci = imageSet(vci,'sensor correction matrix',T);
            img = imageLinearTransform(img,T);  % vcNewGraphWin; imagesc(img)

            % Set the other transforms to empty.
            vci = imageSet(vci,'illuminant correction transform',[]);
            % vci = imageSet(vci,'sensor correction transform',[]);
            vci = imageSet(vci,'ics2display',[]);
        case 'adaptive'
            ...
            % field imageGet(vci,'Sensor Correction Method')
    ...
    img = ieClip(img,0,imageGet(vci,'sensor max'));
    vci = imageSet(vci,'result',img);
    
    % macbethSelect rect handles -- get rid of them
    vci = imageSet(vci,'mccRectHandles',[]);
end
...
if ieNotDefined('combinationMethod')
    combinationMethod = imageGet(vci,'combinationMethod'); 
end
...
img       = imageGet(vci,'input');
sensorMax = imageGet(vci,'sensorMax');
...
vci = imageSet(vci,'sensorMax',sensorMax/min(expTimes));
vci = imageSet(vci,'input',img);
...
vci = imageSet(vci,'sensorMax',mx);
vci = imageSet(vci,'input',newImg);
