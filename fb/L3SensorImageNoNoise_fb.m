function [desiredIm, inputIm] = L3SensorImageNoNoise_fb(L3)
...
% To compute ideal XYZ image, set voltage swing to infinite and
% quantization method to analog to avoid clipping and quantization
sensorMIdeal = sensorM;
pixel = sensorGet(sensorMIdeal,'pixel');
pixel = pixelSet(pixel, 'voltage swing', Inf);
sensorMIdeal = sensorSet(sensorMIdeal, 'pixel', pixel);
sensorMIdeal = sensorSet(sensorMIdeal,'quantizationMethod','analog');
...
for ii=1:nScenes
    ...
    desiredIm{ii} = monoCompute(sensorMIdeal,oi,cFilters);
end
