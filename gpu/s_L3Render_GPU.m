...
% fName = sprintf('C:\\Users\\Haomiao\\Desktop\\L3-tmp\\L3-GPU\\L3-GPU\\video\\output_%07d.raw', 0);
fName  = sprintf('H:\\Film1\\output_%07d.raw', 800);
sensor = fbRead(fName);
...
volts = single(volts - min(volts(:)));
% volts = single(volts / (2^12 -1));
...
lum_list = (lum_list(1:end-1) + lum_list(2:end))/2;
% lum_list = [0 lum_list];
